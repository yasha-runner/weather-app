...
export const getWeatherWeek = (city: ICurrentCity): ThunkAction<void, RootState, null, IWeatherWeekAction> => {
    return async dispatch => {
        try {
            const res = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${city.coord.lat}&lon=${city.coord.lon}&exclude=current,minutely,hourly,alerts&appid=${API_KEY}`);
